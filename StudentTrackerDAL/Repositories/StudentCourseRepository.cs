...
namespace StudentTrackerDAL.Repositories
{
    public class StudentCourseRepository
    {
        ...
        {
            using var con = new SqlConnection(_connectionString);
            var sql = @"UPDATE StudentCourses SET CourseID = @CourseID WHERE StudentCourseID = @StudentCourseID;";
            return await con.ExecuteAsync(sql, new { course.StudentCourseID, course.CourseID });
        }
        ...
        {
            using var con = new SqlConnection(_connectionString);
            var sql = @"SELECT COUNT(*) FROM StudentCourses sc INNER JOIN Users u ON u.UserID = sc.StudentID WHERE sc.CourseID = @CourseID AND u.RoleID = 3;";
